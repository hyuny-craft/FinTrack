...
public class ExpenseService {
    ...
    public TotalExpenseResponse getTotalExpense(String email, YearMonth yearMonth) {
        ...
        BigDecimal total = expenseRepository.findByUserAndDateBetween(user, yearMonth.atDay(1), yearMonth.atEndOfMonth())
                .stream()
    ...
    public CategoryRatioResponse calculateCategoryRatio(String email, YearMonth yearMonth) {
        ...
        BigDecimal total = expenseRepository.sumAmountByUserEmailAndDateBetween(email, start, end);
        if (total.compareTo(BigDecimal.ZERO) == 0) {
            return new CategoryRatioResponse(List.of());
        }

        List<CategoryRatioResponse.CategoryRatioItem> items = expenseRepository.findByUserEmailAndDateBetween(email, start, end)
                .stream()
                .map(row -> new CategoryRatioResponse.CategoryRatioItem(
                        row.getCategory().name(),
                        row.getAmount(),
                        row.getAmount().divide(total, 4, RoundingMode.HALF_UP).doubleValue() * 100
                ))
                .toList();
    ...
    public WeeklyExpenseTrendResponse getWeeklyTrend(String email) {
        ...
        List<WeeklyExpenseTrendResponse.DailExpense> dailyExpenses = expenseRepository
                .findByUserEmailAndDateBetween(email, weekAgo, today)
                .stream()
